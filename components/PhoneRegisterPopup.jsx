...
export default function PhoneRegisterPopup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) { setMessage("✅ تم التسجيل بنجاح!"); setPhone(""); setTimeout(() => closePopup(), 2000); }
      else setMessage("❌ فشل الإرسال");
    } catch (error) { setMessage("❌ حدث خطأ"); }
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[999999]" onClick={closePopup}>
      <div className="bg-white rounded-2xl p-6 w-[90%] max-w-md relative shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="text-center">
          {/* ✅ اللوغو بدلاً من النص */}
          <img src="/mamaty-logo.png" alt="مامتي" className="h-16 mx-auto mb-4" />
          ...
          <input type="tel" placeholder="رقم الهاتف واتساب" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full p-3 border-2 border-gray-200 rounded-xl text-center mb-4" dir="ltr" />
          <button onClick={handleSubmit} disabled={loading} className="w-full bg-orange-500 text-white py-3 rounded-xl font-bold text-lg hover:bg-orange-600 transition disabled:bg-gray-400">
            {loading ? "جاري الإرسال..." : "أخبرني عند وصول الجديد"}
          </button>
          <p className="mt-4 text-sm text-gray-500">🔥 أكثر من 1000 شخص مشترك — رقمك آمن 100%</p>
          {message && <p className={`mt-3 p-2 rounded-lg ${message.includes('✅') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{message}</p>}
        </div>
      </div>
    </div>
  );
