...
const BottomBar = ({ cartCount = 0, onAIClick, onInfoClick, onWhatsAppClick, onSearch }) => {
  ...
  const handleFilterApply = (filters) => {
    ...
    // يمكن ربطه بالصفحة الرئيسية لاحقاً
  };
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-[#FF8A5C]/20 py-2 px-4 z-50 shadow-lg">
        <div className="flex justify-around items-center max-w-md mx-auto">
          ...
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => setIsMenuOpen(true)} className="flex flex-col items-center">
            ...
            <span className="text-xs mt-1 text-black font-medium">القائمة</span>
          </motion.button>
          ...
          {/* السلة */}
          <Link href="/cart">
            <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} animate={{ y: [0, -5, 0, -3, 0] }} transition={{ duration: 2, repeat: Infinity }} className="flex flex-col items-center cursor-pointer relative">
              <div className="relative">
                <img src={cartCount > 0 ? "/icons/bottom-bar/cart-active.png" : "/icons/bottom-bar/cart-default.png"} alt="السلة" className="w-7 h-7 relative z-10" />
                {cartCount > 0 && <span className="absolute -top-2 -right-2 bg-[#FF8A5C] text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center border-2 border-white z-20">{cartCount}</span>}
              </div>
          ...
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => setIsFilterOpen(true)} className="flex flex-col items-center">
            <img src="/icons/filter.png" alt="فرز" className="w-6 h-6" />
            <span className="text-xs mt-1 text-gray-600">فرز</span>
          </motion.button>

          {/* واتساب - بدون دائرة، أيقونة فقط */}
          <motion.button whileTap={{ scale: 0.9 }} onClick={onWhatsAppClick} className="flex flex-col items-center">
            <img src="/icons/whatsapp.png" alt="واتساب" className="w-5 h-5" />
